/*
   ...
   xjia@ust.hk 2015/04/08
   revised by Lipeng WANG  lwangay@connect.ust.hk  OCT. 17 2016
 */
#include <iostream>
#include <cstdio>
...
int main()
{
  ...
  std::rand();
  int numEachParts[numPart];
  memset(numEachParts, 0, sizeof(int)*numPart);
  for(int i = 0; i < r_len; ++i){
	 h_R[i] = std::rand() % 100;
   numEachParts[h_R[i]/10]++;
 }
  ...
  int pointer = 0;
  for(int i = 0 ; i < numPart; i++){
    for(int j = pointer; j<pointer+numEachParts[i];j++){
      if((h_result[j] / 10) != i){
        fprintf(stderr, "Error! [%d] -> %d, should in %d part not %d.\n", j, h_result[j], h_result[j]/10, i);
        return 1;
      }
    }
    pointer += numEachParts[i];
  }
